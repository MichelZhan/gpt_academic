import os
import gradio as gr
from redis_utils import RedisAuth
from mongo_history import MongoHistory
os.environ['no_proxy'] = '*'


def main(r_auth: RedisAuth, m_history: MongoHistory):
    ...
    title_html = "<h1 align=\"center\">AcademicGPT</h1> <!-- RackNerd -->"
    description = """Welcome to AcademicGPT"""
    ...
    cancel_handles = []
    with gr.Blocks(title="AcademicGPT 🤔", theme=set_theme, analytics_enabled=False, css=advanced_css) as demo:
        gr.HTML(title_html)
        ...
        with gr_L1():
            with gr_L2(scale=2):
                ...
                with gr.Accordion("输入区", open=True) as area_input_primary:
                    with gr.Row():
                        txt = gr.Textbox(show_label=False, placeholder="按Enter提交, 按Shift+Enter换行").style(
                            container=False)
                    with gr.Row():
        ...
        # 整理反复出现的控件句柄组合
        input_combo = [cookies, max_length_sl, md_dropdown, txt, txt, top_p, temperature, chatbot, history,
        ...
        clearBtn.click(lambda: ("", ""), None, [txt, txt])
    ...
    auto_opentab_delay()
    demo.queue(concurrency_count=CONCURRENT_COUNT, api_open=False).launch(auth_message="Welcome to AcademicGPT",
                                                                          server_name="0.0.0.0", server_port=PORT,
                                                                          auth=r_auth.auth, share=False, show_api=False,
...
if __name__ == "__main__":
    ...
    mongodb_history.start()
    gr.close_all()
    main(redis_auth, mongodb_history)
    mongodb_history.stop()
